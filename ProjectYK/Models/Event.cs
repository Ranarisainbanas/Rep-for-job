...
namespace ProjectYK.Models
{
    public class Event
    {
        ...
        public DateTime Finish_Day { get; set; }    //Дата завершения мероприятия. Если мероприятие повторяющееся, то указывается день, после которого событие больше не произойдет.
        public int Recurs { get; set; }    //Тип повторения: 0 - без повторения, 1 - ежедневно, 2 - еженедельно, 3 - ежемесечно
        //Если Recurs = 0 , то поля ниже тоже = 0
        ...
        Для еженедельных событий можно использовать несколько дней недели (каждые 3 недели во вторник и четврег - Recurs=2, Recurs_Interval=3 Recurs_Day_of_Week=4+16=20).
        Выбранные значения позволяют комбинировать дни недели.
