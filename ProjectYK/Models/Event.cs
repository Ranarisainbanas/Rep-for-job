...
namespace ProjectYK.Models
{
    public class Event
    {
        ...
        public Byte Recurs { get; set; }    //Тип повторения: 0 - без повторения, 1 - ежедневно, 2 - еженедельно, 3 - ежемесечно
        //Если Recurs = 0 , то поля ниже тоже = 0, кроме даты завершения события - она равна дате начала
        public Int16 Recurs_Interval { get; set; }  //Интервал между повторяющимеся событиями
        ...
        public Byte Recurs_Day_Number { get; set; } //Тип повторения, при котором задается число месяца. Используется только при Recurs=3, иначе значение 0. Например, 13го числа каждого месяца - Recurs=3, Recurs_Interval=1, Recurs_Day_Number=13
        public Byte Recurs_Serial_Day { get; set; } //Тип повторения, при котором задается порядковый номер дня месяца: 1 - первый; 2 - второй; 3 - третий; 4 - четвертый; 5 - последний.
        public Byte Recurs_Day_of_Week { get; set; }    //Дни недели. 1 - Воскресенье, 2 - Понедельник, 4 - Вторник, 8 - Среда, 16 - Четверг, 32 - Пятница, 64 - Суббота.
